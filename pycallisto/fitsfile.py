...
class ECallistoFitsFile(FitsFile):
    ...
    def plot_db_above_background(self, show=False, save=True):
        ...
        plt.imshow(
            self.hdul_dataset["db"] - self.hdul_dataset["db_median"],
            cmap="magma",
            norm=plt.Normalize(self.hdul_dataset["v_min"], self.hdul_dataset["v_max"]),
            aspect="auto",
            extent=[
                self.hdul_dataset["time_axis"][0],
    ...
    def plot_freq_range_db_above_background(
        self, start_freq, end_freq, show=False, save=True
    ):
        plt.figure(1, figsize=(11, 6))
        plt.imshow(
            self.hdul_dataset["db"] - self.hdul_dataset["db_median"],
    ...
    def plot_fits_files_list(
        ...
        plt.savefig(
            os.path.join(os.getcwd(), plot_filename) + ".png", bbox_inches="tight"
        )
    ...
    def set_fits_linear_regression(self):
        ...
        hdul_dataset["lin_reg"] = np.polyfit(
            hdul_dataset["time_axis"], hdul_dataset["freq_axis"], 1
        )
    ...
    def fits_list_linear_regression(file_list):
        ...
        for name in file_list:
            ...
            if extended_time is None and extended_freq is None:
                ...
                extended_time = np.hstack((extended_time, dataset["time_axis"]))
                extended_freq = np.hstack((extended_freq, dataset["freq_axis"]))
    ...
    def plot_fits_linear_regression(self, show=False, save=True):
        ...
        plt.title(
            ...
            + f"{intercept:.2f} + ({slope:.2f}t)",
            fontsize=16,
        )
        plt.tick_params(labelsize=14)
        if save:
            img_filename = (
                ".".join(self.filepath.split(".")[:-2]) + "linear_regression.png"
            )
            plt.savefig(img_filename, bbox_inches="tight")
        if show:
            plt.show()
    ...
    def plot_lin_reg_above_spectrum(
        self, figsize=(11, 6), dpi=200, labelsize=None, show=False, save=True, **kwargs
    ):
        self.set_fits_linear_regression()
        self.set_fits_linear_regression_function()
        hdul_dataset = self.hdul_dataset
        figsize = (11, 6)
        plt.figure(1, figsize, dpi)
        plt.imshow(
            hdul_dataset["db"] - hdul_dataset["db_median"],
            "magma",
            plt.Normalize(hdul_dataset["v_min"], hdul_dataset["v_max"]),
            "auto",
            extent=[
        ...
        plt.gca().invert_yaxis()
        plt.colorbar().set_label("dB above background")
        plt.xlabel("Time (UT)", fontsize=15)
        plt.ylabel("Frequency (MHz)", fontsize=16)
        if labelsize is not None:
            plt.tick_params("both", labelsize=labelsize)

        if show:
            plt.show()
        if save:
            img_filename = ".".join([pathlib.Path(self.filename).stem, "png"])
            plt.savefig(img_filename, bbox_inches="tight")
